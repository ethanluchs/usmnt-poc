...
export function useGameState() {
    ...
    const revealedCountryCodes = new Set(
        player?.careerStops.slice(0, currentStop + 1).map(s => s.countryCode) ?? []
    )
    ...
    return { player, puzzleIndex, currentStop, incorrectGuesses, solved, revealedCountryCodes, onGuess, onNextStop, onNextPuzzle }
}
